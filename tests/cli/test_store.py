...
def test_run_copies_matched_file(
    ...
    monkeypatch.setattr("fintl.cli.commands.store.Config", lambda: config)
    monkeypatch.setattr("fintl.cli.commands.store.ALL_PARSERS", [spec])
...
def test_run_ambiguous_with_yes_skips(
    ...
    monkeypatch.setattr("fintl.cli.commands.store.Config", lambda: config)
    monkeypatch.setattr(
        "fintl.cli.commands.store.ALL_PARSERS", [spec_giro, spec_credit]
    )
