...
from fintl.accounts_etl.providers.dkb import tagesgeld202312 as tagesgeld
...
def test_parse_csv_file_raises_extract_transactions_exception():
    with patch(
        "fintl.accounts_etl.providers.dkb.tagesgeld202312.extract_transactions",
...
def test_parse_csv_file_raises_extract_balance_exception():
    with patch(
        "fintl.accounts_etl.providers.dkb.tagesgeld202312.extract_balance",
...
def test_parse_new_files_skips_failing_file_and_continues(tmp_path: Path):
    ...
    with (
        patch(
            "fintl.accounts_etl.providers.dkb.tagesgeld202312.parse_csv_file",
            side_effect=_parse_csv_file,
        ),
        patch(
            "fintl.accounts_etl.providers.dkb.tagesgeld202312.store_transactions"
        ) as mock_store_t,
        patch(
            "fintl.accounts_etl.providers.dkb.tagesgeld202312.store_balance"
        ) as mock_store_b,
    ):
