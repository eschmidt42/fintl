...
from fintl.etl.providers.gls import credit0 as credit
from fintl.etl.providers.gls import helper
from fintl.etl.providers.gls.credit0 import CASE
...
def test_parse_new_files_happy_path(tmp_path: Path, csv_file: Path):
    ...
    from fintl.etl.providers.gls import credit0

    parsed_dir = tmp_path / "parsed"
    credit0.parse_new_files(credit0.CASE, [csv_file], parsed_dir)
...
def test_parse_new_files_error_propagates(tmp_path: Path):
    ...
    with patch(
        "fintl.etl.providers.gls.credit0.parse_csv_file",
        side_effect=ValueError("parse failed"),
    ):
        with pytest.raises(ValueError, match="parse failed"):
            credit0.parse_new_files(credit0.CASE, [file1], parsed_dir)
