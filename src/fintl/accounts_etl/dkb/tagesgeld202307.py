...
from fintl.accounts_etl.files.files_utils import (
    copy_new_files,
    load_lines,
...
CASE = Case(
    ...
    parser=DKBTagesgeldParserEnum.tagesgeld202307.value,
)
...
def check_if_parser_applies(file_path: Path) -> bool:
    ...
    is_header_match = any(
        re.search(
            r'"Buchungsdatum";"Wertstellung";"Status";"Zahlungspflichtige\*r";"Zahlungsempfänger\*in";"Verwendungszweck";"Umsatztyp";"Betrag";"Gläubiger-ID";"Mandatsreferenz";"Kundenreferenz"',
            line,
        )
        for line in lines
    )
...
def extract_transactions(
    ...
    transactions = transactions.with_columns(
        ...
        source=pl.col("Zahlungspflichtige*r"),
        recipient=pl.col("Zahlungsempfänger*in"),
