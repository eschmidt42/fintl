...
def test_parse_new_files_skips_failing_file_and_continues(tmp_path: Path):
    ...
    with (
        patch(
            "fintl.accounts_etl.providers.dkb.credit0.parse_csv_file",
            side_effect=_parse_csv_file,
        ),
        patch(
            "fintl.accounts_etl.providers.dkb.credit0.store_transactions"
        ) as mock_store_t,
        patch("fintl.accounts_etl.providers.dkb.credit0.store_balance") as mock_store_b,
    ):
