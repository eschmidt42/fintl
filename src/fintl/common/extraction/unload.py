...
def _get_ollama_root_url(base_url: str) -> str:
    """Return the ollama root URL, stripping any trailing /v1 suffix."""
    url = base_url.rstrip("/")
    return url.removesuffix("/v1")


def _list_running_ollama_models(root_url: str, timeout: int) -> list[str]:
    """Return model names currently loaded in ollama via GET /api/ps."""
    try:
        r = httpx.get(f"{root_url}/api/ps", timeout=timeout)
        r.raise_for_status()
...
def unload_ollama(config: OllamaConfig | None, timeout: int):
    ...
    try:
        check_ollama_availability(base_url)
    except OllamaUnavailableError:
        logger.debug("Ollama is not running, no need for further steps.")
        return

    logger.debug("Confirmed ollama is running.")
    root_url = _get_ollama_root_url(base_url)
    generate_url = f"{root_url}/api/generate"

    logger.debug("Discovering running models via /api/ps.")
    models_to_unload = _list_running_ollama_models(root_url, timeout)

    for model in models_to_unload:
        logger.debug("Attempting unloading of ollama model '%s' via '%s'.", model, generate_url)
        try:
            r = httpx.post(
                generate_url,
                json={
